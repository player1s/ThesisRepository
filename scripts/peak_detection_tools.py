...
def ensemble_peak(preprocessed_data, fs, ensemble_ths=4,margin=1):
    ...
    #peak voting: if peak is already in the dict, add 1 if not register it with one vote
    for key in peak1:
        peak_dic[key] = 1
    ...
    for key in peak_dic.keys():
        if cnt == 0:
            ...
            if bef_key + margin >= key:  # If the next peak exists within margin 1
                if peak_dic[bef_key] > peak_dic[key]: # If the previous key was considered a peak by more algorithms. Keep previous
                    new_peak_dic[bef_key] = new_peak_dic[bef_key]
                else:
    ...
    return final_peak # Returns a list with the position of the chosen peaks
