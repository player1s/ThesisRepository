...
import peak_detection_tools as pk
...
def calc_fd_hrv(RR_list):  
    ...
    if len(rr_x) <= 3 or len(RR_list) <= 3:
        print("rr_x or RR_list less than 3")   
        return 0
...
def calc_heartrate(RR_list):
    ...
    for val in RR_list:
        ...
        # if RR-interval < .1905 seconds, heart-rate > highest recorded value, 315 BPM. Probably an error!
            ...
            print("in HR calculation, division by 0")
            heart_rate = 0.0
...
def get_window_stats_27_features(ppg_seg, window_length, label, ensemble, ma_usage,margin):  
    
    fs = 64
    
    if ma_usage: # replaces the original signal by the mean of the fordward and backwards moving average
        fwd = pk.moving_average(ppg_seg, size=3)
        bwd = pk.moving_average(ppg_seg[::-1], size=3)
    ...
    peak = pk.lmm_peakdetection(ppg_seg,fs)

        
    if ensemble:
        ensemble_ths = 3
        #print("one algorithm peak length: ", len(peak))
        peak = pk.ensemble_peak(ppg_seg, fs, ensemble_ths,margin)
