...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\mindenJoSHasztalan\\UniversityAndStuff\\DTU\\Semester 6\\Thesis\\Code\\ThesisRepository\\psychopyExperiments\\ASSR experiment\\ASSR2_lastrun.py',
...
text = visual.TextStim(win=win, name='text',
    text='Welcome to the experiment.\nParticipant: Sit upright, try to relax, and please avoid during data collection: \n- unnecessary movements\n- talking\n\nTester: \n- set volume to 40\n- do impedance and railing checks\n- on labrecorder set: study root, block/task, run, participant, session\n- gather both streams and start labrecorder recording\n- this version of the test measures at: F8, F7 (original paper), T3, T4 (closeness to headset measurement locations), T6, T5 (many papers point to temporal / parietal regions), C3, C4 (closest location to primary auditory cortex, which is said to be the source of assr)  Reference at: Fz, Ground at: Pz\npress space to continue',
    font='Open Sans',
...
for thisTrial in trials:
    ...
    # set up handler to look after randomisation of conditions etc
    trials_2 = data.TrialHandler(nReps=25.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
    ...
    for thisTrial_2 in trials_2:
        ...
        routineForceEnded = False
        # update component parameters for each repeat
        ModulatedTone.setSound('1kHzAmplitudeModulated40Hz.wav', secs=10, hamming=True)
    ...
    # --- Ending Routine "Impedance_check" ---
    for thisComponent in Impedance_checkComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # check responses
...
_key_resp_2_allKeys = []
# keep track of which components have finished
OutroComponents = [text_4, key_resp_2]
...
while continueRoutine:
    ...
    if text_4.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > text_4.tStartRefresh + 1000.0-frameTolerance:
            # keep track of stop time/frame for later
