...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='A cross will appear, focus on that. You will hear a tone. \n\n30 s rest\n30 s tone',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 70.0:
            ...
            if text_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > text_2.tStartRefresh + 9-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # if ModulatedTone is starting this frame...
            if ModulatedTone.status == NOT_STARTED and tThisFlip >= 40-frameTolerance:
                # keep track of start time/frame for later
                ...
                # update status
                ModulatedTone.status = STARTED
                ModulatedTone.play(when=win)  # sync with win flip
            
            # if ModulatedTone is stopping this frame...
            if ModulatedTone.status == STARTED:
                ...
                if tThisFlipGlobal > ModulatedTone.tStartRefresh + 30-frameTolerance:
                    ...
                    # update status
                    ModulatedTone.status = FINISHED
                    ModulatedTone.stop()
            # update ModulatedTone status according to whether it's playing
            if ModulatedTone.isPlaying:
                ModulatedTone.status = STARTED
            elif ModulatedTone.isFinished:
                ModulatedTone.status = FINISHED
            
            # *Cross* updates
            
            # if Cross is starting this frame...
            if Cross.status == NOT_STARTED and tThisFlip >= 9-frameTolerance:
                # keep track of start time/frame for later
            ...
            if Cross.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Cross.tStartRefresh + 61-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if ModulatedTone.status == FINISHED and flag_end == True:
                print('end')
                flag_end = False
                outlet.push_sample(x=[endASSR])
            
            if text_3.status == STARTED and flag_start == True:
                print('start')
                flag_start = False
                outlet.push_sample(x=[startRest])
            
            if text_3.status == FINISHED and flag_end == True:
                print('end')
                flag_end = False
            ...
            if text_3.status == NOT_STARTED and tThisFlip >= 9-frameTolerance:
                # keep track of start time/frame for later
        ...
        for thisComponent in ASSRComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('ASSR.stopped', globalClock.getTime())
        ModulatedTone.pause()  # ensure sound has stopped at end of Routine
