...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\mindenJoSHasztalan\\UniversityAndStuff\\DTU\\Semester 6\\Thesis\\psychopy\\oddball-task-master\\Oddball Task_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    if key_resp_1.keys != None:  # we had a response
        ...
        thisExp.addData('key_resp_1.duration', key_resp_1.duration)
    thisExp.nextEntry()
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if response.status == STARTED and not waitOnFlip:
                theseKeys = response.getKeys(keyList=['s','t'], ignoreKeys=["escape"], waitRelease=False)
    ...
    # set up handler to look after randomisation of conditions etc
    trials_2 = data.TrialHandler(nReps=10, method='fullRandom', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('aud_conditions.xlsx'),
    ...
    while continueRoutine:
        ...
        if exit_exp.status == STARTED and not waitOnFlip:
            theseKeys = exit_exp.getKeys(keyList=['e'], ignoreKeys=["escape"], waitRelease=False)
